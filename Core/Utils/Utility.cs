...
namespace Core.Utils
{
    public class Utility
    {
       ...
       "àáảãạâầấẩẫậăằắẳẵặèéẻẽẹêềếểễệđìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵÀÁẢÃẠÂẦẤẨẪẬĂẰẮẲẴẶÈÉẺẼẸÊỀẾỂỄỆĐÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴÂĂĐÔƠƯ";
        ...
        private const string DangerousPattern = "</?[a-z0-9]+[^>]*/?>|onerror|script|prompt|throw|try|catch|alert|function|table|truncate|select|drop|delete|insert|create";
        public static string RemoveDangerousTag(string input)
        {
            if (string.IsNullOrEmpty(input))
            {
                return string.Empty;
            }
            var regex = new Regex(DangerousPattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);
            var result = regex.Replace(input, string.Empty);
            result = Regex.Replace(result, "[\r\n\t]", " ").Trim();
            return FormatQuery(result);
        }
        private const string Patern = "[~\\*\\?:+\\-\\|\\!{}\\(\\)\\[\\]\\^\\<>\\\\\"#]";
        public static string FormatQuery(string input)
        {
            input = Regex.Replace(input, Patern, "");
