<template>
    <div class="ui-builder-item" :class="{ draggable: draggable, resizable: resizable, active: active }" @mousedown="elmDown"
        ...
        <template v-if="resizable">
            <div class="handle"
                 ...
                 @mousedown.stop.prevent="handleDown(handle, $event)" :key="index"></div>
        </template>
        <slot></slot>
    </div>
</template>
        ...
        methods: {
            ...
            deselect: function (e) {
                ...
                let regex = new RegExp('handle-([trmbl]{2})', '')
                 let regex2 = new RegExp('handle-rot', '')

                if (target !== this.$el && !regex.test(target.className) && !regex2.test(target.className)) {
                    if (this.active) {
                        this.active = false
...
<style scoped>
    ...
    .ui-builder-item { position: absolute; box-sizing: border-box; border: 0; }
        .ui-builder-item.active { border: dashed 1px #ccc; }


    .draggable:hover { cursor: move; }

    .handle { box-sizing: border-box; display: none; position: absolute; width: 10px; height: 10px; font-size: 1px; background: #EEE; border: 1px solid #333; }
    ...
    .handle-ml { top: 50%; margin-top: -5px; left: -10px; cursor: w-resize; }

    .handle-mr { top: 50%; margin-top: -5px; right: -10px; cursor: e-resize; }

    .handle-bl { bottom: -10px; left: -10px; cursor: sw-resize; }

    .handle-bm { bottom: -10px; left: 50%; margin-left: -5px; cursor: s-resize; }

    .handle-rot { bottom: -45px; left: 50%; margin-left: -5px; -webkit-border-radius: 50%; -moz-border-radius: 50%; border-radius: 50%; cursor: pointer; width: 20px; height: 20px; }
        .handle-rot:before { content: ''; position: absolute; left: 50%; top: -20px; width: 0; height: 20px; margin-left: -1px; border-left: 1px dashed #3f4652; background-color: #fff; }
        .handle-rot:after { content: ''; background-color: #fff; background-image: url(https://cdn.rawgit.com/berkaygure/19b8f05102fb11f57fff1cf3f3a2fc48/raw/6f7a11dd1ea612ce7392c1b6d7ccbabb6404e489/icon_rotate.svg); background-size: 14px 14px; background-repeat: no-repeat; background-position: 1px 2px; position: absolute; left: 1px; top: 1px; width: 16px; height: 16px; border-radius: 16px; box-shadow: 0 0 3px 1px rgba(0,0,0,.15); }
